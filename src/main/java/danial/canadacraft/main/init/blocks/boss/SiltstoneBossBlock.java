...
public class SiltstoneBossBlock extends Block {
		   
	public SiltstoneBossBlock(Properties properties) {
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
		...
		Entity maplebossentity = CanadaEntities.SILTSTONE_BOSS_ENTITY.create(worldIn);
		...
		for(int i = 0; i < 3; i++) {
			...
			Entity maplemonsterentity = CanadaEntities.MINER.create(worldIn);
			maplemonsterentity.setLocationAndAngles((double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, 0.0F, 0.0F);
			worldIn.addEntity(maplemonsterentity);
