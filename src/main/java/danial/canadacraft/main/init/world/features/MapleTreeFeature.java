...
public class MapleTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
	...
	private final BlockState LOG = CanadaBlocks.maple_wood_log.getDefaultState();
	private static BlockState LEAF = CanadaBlocks.maple_leaves.getDefaultState();
	private final boolean useExtraRandomHeight;

	public MapleTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configIn, boolean doBlockNotifyIn, boolean extraRandomHeightIn) {
		
		super(configIn, doBlockNotifyIn);
		this.useExtraRandomHeight = extraRandomHeightIn;
		this.setSapling((net.minecraftforge.common.IPlantable) CanadaBlocks.maple_sapling);
			...
			if (!flag) {
				...
				for (int i2 = 0; i2 < i; ++i2) {
					...
					if (isAirOrLeaves(worldIn, position.up(i2))) {
						...
						this.placeTrunk(this.LOG, changedBlocks, worldIn, position.up(i2), 3, p_208519_5_);
	...
	protected void placeTrunk(BlockState log, Set<BlockPos> changedBlocks, IWorldGenerationReader world, BlockPos pos, int height, MutableBoundingBox boundingBox) {
		
		for (int y = 0; y < height; y++) {
			
			this.setLogState(changedBlocks, world, pos, log, boundingBox);
