...
public class TremblingAspenTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
	
	private static final BlockState TRUNK = CanadaBlocks.trembling_aspen_wood_log.getDefaultState();
	private static final BlockState LEAF = CanadaBlocks.trembling_aspen_leaves.getDefaultState();

	public TremblingAspenTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i51429_1_, boolean p_i51429_2_) {
		
		super(p_i51429_1_, p_i51429_2_);
		setSapling((net.minecraftforge.common.IPlantable) CanadaBlocks.trembling_aspen_sapling);
					...
					for (int i2 = position.getX() - i3; i2 <= position.getX() + i3; ++i2) {
						...
						for (int k2 = position.getZ() - i3; k2 <= position.getZ() + i3; ++k2) {
							...
							if (Math.abs(j2) != i3 || Math.abs(l2) != i3 || i3 <= 0) {
								...
								if (isAirOrLeaves(worldIn, blockpos) || CanadaAbstractTreeFeature.func_214576_j(worldIn, blockpos)) {
